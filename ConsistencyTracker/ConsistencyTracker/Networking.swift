...
enum TypeOfPackage: Int {
    ...
    case stop = 2
    case motionData = 3
    case enableVideoCaptureModule = 4
}
...
class TCPConnection {
    ...
    func start(delegate: TCPConnectionDelegate, queue: DispatchQueue) {
        ...
        self.connection.stateUpdateHandler = {state in
            ...
            case .failed(let err):
                debugPrint("Connection lost, Error \(err)")
    ...
    private func sendData(packageTypePrefix: Data, data: Data, completionHandler: (() -> Void)?) {
        ...
        self.connection.batch {
            ...
            self.connection.send(content: sizePrefix, completion: .contentProcessed({err in
                if let err = err {
                    debugPrint("Connection Error while sending sizePrefix: \(err)")
                    self.delegate?.connectionError(error: err)
                }
            }))
            self.connection.send(content: data, isComplete: true, completion: .contentProcessed({err in
                if let err = err {
                    debugPrint("Connection Error while sending data: \(err)")
                    self.delegate?.connectionError(error: err)
                } else {
                    completionHandler?()
                }
    ...
    private func sendData(packageTypePrefix: Data, data: [Data], completionHandler: (() -> Void)?) {
        var compHandler: (() -> Void)? = {}
        self.connection.batch {
            for (i, dat) in data.enumerated() {
                if i == data.count - 1 {
                    compHandler = completionHandler
                }
                sendData(packageTypePrefix: packageTypePrefix, data: dat, completionHandler: compHandler)
            }
        }
    }
